...
> $$oxed{\;\Delta L(p) = \deg(p) - k - 1 - \delta^{+}(p)\;}$$
...
$$\Delta L = \deg(p) - 2k + (k-1) - \delta^{+} = \deg(p) - k - 1 - \delta^{+} \qquadlacksquare$$
...
所以扣掉 $p$ 自己之後，第三項只增不減，即 $\delta^{+} \ge 0$。$lacksquare$
...
程式跑 6,000 個隨機形狀、對每個形狀的每一個相鄰空點都試一次：實際出現的最大 $\Delta L$ **恰好是 2**，而且 $\delta^{+} = 0$（上界達到）的情況佔了四成以上。上界是緊的，而且一點都不罕見 —— 「乾淨地長一手」正是實戰上最常見的一手棋。
