...
**實驗二：任意的小賽局。** 放寬到隨機生成的任意賽局樹（不限於圍棋會出現的形狀），2,184 組：

| 局部裡挑哪一手的判準 | 非最優 | 比例 | 最大落差 |
| :--- | ---: | ---: | ---: |
| 挑**均值**最好的（錯的判準） | 50 組 | 2.3% | $\tfrac52$ |
| 挑**熱圖**指定的（對的判準） | 36 組 | 1.6% | $2$ |

兩件事同時成立：**貪婪法不是永遠最優**，而且**局部裡挑哪一手的判準會影響結果**。

**實驗三：落差有多大？** 對同樣 2,184 組，比較落差與 $t_{\max}$（全盤最高溫度）：
...
>
> **這是一個經驗結果，不是本書證明的定理。** 上面 2,184 組隨機賽局全部滿足，而且上界是緊的（有落差剛好等於 $t_{\max}$ 的例子）。這個界在 Berlekamp 與 Wolfe 的著作裡有理論版本；本書只驗證，不重證。
...
* **「大的先下」是一條帶誤差界的規則**：落差 $\le t_{\max}$（2,184 組隨機驗證，上界是緊的）。而在圍棋實際會出現的局部上（開關與走廊，600 組），它就是**最優解**。
