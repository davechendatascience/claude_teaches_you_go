...
```text
   ...
   1 . X . . . . . 1
     A B C D E F G
```
*白 A3、A4 這兩顆做不出兩個眼 —— 大家都同意它們死了。但【同意】不是算術。中國規則不必判：黑下 a 或 b 提掉就是了。日本規則必須判，所以它需要一套確認手續。*
...
黑下 A5（或 A2）就提掉了。黑再把剩下的點填一填，整個 49 點的棋盤都是黑的：

$$C = 49$$

**沒有人需要「判定」任何東西** —— 死子是被**提**掉的，不是被**宣告**掉的。
代價是要多下幾手（本例是 5 手），而在數子規則下那幾手不花錢。
