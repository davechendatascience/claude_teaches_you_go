...
> [!RECALL]
> **定義 5.5：$K$ 是「打贏」和「打輸」之間的差距，不是「這塊棋值多少」。**
...
```text
   ...
   1 . . . . . . . . . 1
     A B C D E F G H J
```
*練習 5.1：黑下 a（E5）提掉白 D5 之後，白【不能】立刻回提 D5。但白可以下別的地方。請說出：白下完別處、黑應了之後，白能不能回提 D5？三種規則（基本劫規、positional、situational）的答案一樣嗎？*
...
* **基本劫規**：它只禁止「下出上一手之前那個盤面」。白下完別處、黑應了之後，盤面上多了兩顆子 —— 這時白回提 D5 產生的盤面，和劫開始前那個盤面**不一樣**（多了那兩顆子）。所以合法。
...
```text
   ...
   1 . X O . . . O X . 1
     A B C D E F G H J
```
*練習 5.3：這個三劫盤面，在 situational superko 之下，黑提了左下的劫以後，白最多能連續提幾次劫才會被規則擋住？*
