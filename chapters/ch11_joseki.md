...
graph TD
    ...
    F2["<b>失效二：劫</b><br/>同盤面同輪次，V 最多差 18 目<br/>72/196 組<br/><i>4.1</i>"]:::meas
...
把 3 路盤上 7 手之內能到、而且還有合法手的局面全部掃一遍：有 196 組「同一個盤面、同一方走，但劫狀態不同」，其中 **72 組（37%）的 $V$ 真的不一樣**。

而且差距不只 18 目那一種 —— 實際量到的落差是 $6, 8, 10, 12, 18$ 目五種，**18 目是最大的那一種，不是典型的那一種**。
...
把 3 路盤 7 手之內可達、且還有合法手的局面全掃一遍：有 196 組「同盤面、同輪次、劫狀態不同」，其中 **72 組（37%）的 $V$ 真的不同**（落差 $6$ 到 $18$ 目）。這不是罕見的邊角情況。
...
* **三個失效條件**：有提子（盤面真的不同）、有劫（盤面相同但 $V$ 不同，3 路盤上 72/196 組如此，落差 $6$ 到 $18$ 目）、雙方手數改變。
