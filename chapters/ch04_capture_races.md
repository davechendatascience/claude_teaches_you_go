...
```text
   ...
   1 X X X X 1
     A B C D
```
*a 是全盤唯一的空點，也是雙方唯一的氣（外氣都是 0，公氣 1）。誰先下 a，誰就提掉對方 —— 因為提子發生在自殺判定之前。*

$(a, b, c) = (0, 0, 1)$。黑先下 $a$：先檢查白棋，白的氣歸零 → 提走；提完之後黑那顆子有一大片空間。合法，黑勝。白先同理。
...
```text
   ...
   1 O O O O 1
     A B C D
```
*黑三子被白完全包住，雙方的外氣都是 0，公氣有三口（a、b、c）。白先填任何一口，黑就跟著填，最後白會先沒氣。所以白不能動 —— 雙活。*

$(a, b, c) = (0, 0, 3)$，$d = 1$。判定式說 $\max(c,1) = 3 > 1$，所以不是我勝；而 $2 - c = -1 \le 1 \le 2 = c - 1$，落在雙活帶裡。

用手算一遍：白填 $a$，雙方各剩 2 口；黑填 $b$，各剩 1 口；白現在要填 $c$ 嗎？填了白自己也歸零，而黑也歸零 —— 誰被提？**填的人先提對方**，所以白填 $c$ 會提掉黑。等一下，那白不是贏了嗎？

不對。倒回去看第二步：黑填 $b$ 之後雙方各剩 1 口氣（就是 $c$）。**輪到白**。白填 $c$ → 黑歸零，被提 → 白勝。所以黑不該填 $b$。

黑的正確下法是**不動**。白填 $a$（各剩 2），黑不動，白再填 $b$（各剩 1），此時輪到黑，黑填 $c$ → 提白。所以白也不該填。
...
```text
   ...
   1 X X X O 1
     A B C D
```
*黑有一個眼 e，白沒有。雙方的外氣都是 0，公氣是 a 與 b 兩口。黑贏 —— 而且不管誰先手。兩口公氣全部算在有眼的一方頭上。*

$(a, b, c) = (0, 0, 2)$，黑有一個眼，白沒有。用定理 4.4 算（假裝沒有眼）會得到雙活。但實際上黑贏，而且**不管誰先手**。

為什麼？看白的處境。白要吃黑，必須填掉黑的所有氣：$a$、$b$、還有那個眼 $e$。但 $e$ 是眼 —— 白只有在黑的其他氣都沒了的時候才填得下去（第 3 章命題 3.4）。所以白必須先填 $a$ 和 $b$。

而 $a$、$b$ 是**公氣**：白每填一口，白自己的氣也少一口。白填完 $a$、$b$ 之後，白自己剩幾口氣？零。白在填完之前就被提了。

反過來看黑：黑填 $a$、$b$ 之後，黑自己還剩那個眼 $e$，**一口氣**。夠了。
...
```text
   ...
   1 X O O e 1
     A B C D
```
*這次換白有眼 e。同樣的道理，唯一那口公氣 a 算給白，白勝。把這張圖和上一張並排看：眼在誰那邊，公氣就在誰那邊。*
...
```text
   ...
   1 X X O f 1
     A B C D
```
*雙方各有一個眼（e 與 f），還剩一口公氣 a。誰填 a 誰就先把自己填到只剩一個眼，然後被對方提。所以雙活。「雙方有眼也會雙活」—— 而且這是最常見的一種雙活。*
...
```text
   ...
   1 . . . . . . . 1
     A B C D E F G
```
*黑被白完全包圍，只剩 a、b、c 三點的大眼。白要下幾手才提得掉？先猜一個數，§4.4 會給兩個答案 —— 而且兩個都對。*

口訣說「三目三氣」。我們來實際數一遍。

**白第一手下 $b$（正中）。** 白那顆子的氣是 $a$ 和 $c$。
**黑不動**（動了只是自填）。
**白第二手下 $a$。** 白 $\{a, b\}$ 這一串的氣只剩 $c$。
**現在輪到黑。** 黑下 $c$ 會怎樣？$c$ 是白串的最後一口氣 —— 依「先提對方」，白兩子被提走！黑用一手提掉兩子。

提完之後，眼位裡有一顆黑子（在 $c$），$a$、$b$ 兩點空著。黑的眼位縮小成兩點。

**白第三手下 $a$，第四手下 $b$** —— 第四手填掉黑的最後一口氣，提掉黑棋。
...
花五有一個值得注意的細節：中心那一點 $c$ **不貼著任何黑子**（它的四個鄰點全是眼位裡的其他點）。所以 $c$ 不是黑棋的氣。
...
其中 $b$ 是分枝因子（每一步有幾種選擇），$d$ 是深度。$b = 5$、$d = 6$ 就已經是一萬五千個節點 —— 遠超過人的工作記憶。
...
```text
   ...
   1 X . O 1
     A B C
```
*練習 4.2：把最小雙活改一下 —— 白少一子，多出 a 這個點。先分類 (a, b, c) 並套判定式，然後【檢查三個前提】。最後用搜尋核對。三個答案會不會一致？*
...
交集 $= \{$B1, B2$\}$ → $c = 2$。黑的外氣 $= \{$B3$\}$ → $a = 1$。白的外氣 $= \{$C3$\}$（也就是 $a$ 那一點）→ $b = 1$。
...
```text
   ...
   1 . . . . . . . 1
     A B C D E F G
```
*練習 4.3：同樣的三目大眼。白第一手應該下 a（正中），還是下旁邊？兩種都算一遍，比較白總共要花幾手。*
...
**白先下 $a$（正中，D4）。** 白一子的氣是 C4 與 E4，兩口。黑不動。白下 C4 → 白兩子只剩 E4 一口氣 → 黑下 E4 提掉白兩子。眼位變成只有黑一子在 E4、C4 與 D4 空著（兩點）。白再下 C4、D4 兩手 → 提掉黑。
...
* **讀棋是 AND-OR 樹**，工作量 $b^d$。手筋壓 $b$，已知結論壓 $d$。「讀不清楚」是容量問題，不是聰明問題。
