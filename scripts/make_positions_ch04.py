...
POSITIONS = {
    ...
    "ch04_one_dame": dict(n=4,
        ...
        LB={"C2": "p"},
        comment="p 是全盤唯一的空點，也是雙方唯一的氣（外氣都是 0，公氣 1）。"
                "誰先下 p，誰就提掉對方 —— 因為提子發生在自殺判定之前。",
                # 標籤用 p 而不用 a：本章的 a 已經是「我的外氣」了。
    ),
    ...
    "ch04_seki3": dict(n=4,
        ...
        LB={"B2": "p", "B3": "q", "B4": "r"},
        comment="黑三子被白完全包住，雙方的外氣都是 0，公氣有三口（p、q、r）。"
                "【誰先填公氣，誰就先輸】—— 因為對方只要【不跟】，"
                "等你把公氣填到剩一口，輪到他提你。所以雙方都不動：雙活。",
    ),
    ...
    "ch04_eye_wins": dict(n=4,
        ...
        AW=["C4", "D1", "D2", "D3", "D4"],
        LB={"A2": "e", "B4": "p", "C2": "q"},
        comment="黑有一個眼 e，白沒有。雙方的外氣都是 0，公氣是 p 與 q 兩口。"
    ...
    "ch04_eye_loses": dict(n=4,
        ...
        LB={"D1": "e", "D3": "p"},
        comment="這次換白有眼 e。同樣的道理，唯一那口公氣 p 算給白，白勝。"
    ...
    "ch04_both_eyes_seki": dict(n=4,
        ...
        AW=["C1", "C2", "D2", "D3", "D4"],
        LB={"A2": "e", "D1": "f", "C3": "p"},
        comment="雙方各有一個眼（e 與 f），還剩一口公氣 p。"
                "誰填 p 誰就先把自己填到只剩一個眼，然後被對方提。所以雙活。"
    ...
    "ch04_nakade3": dict(n=7, AB=w3, AW=o3,
        LB={r3[0]: "p", r3[1]: "q", r3[2]: "r"},
        comment="黑被白完全包圍，只剩 p、q、r 三點的大眼。"
                "白要下幾手才提得掉？先猜一個數，§4.4 會給兩個答案 —— 而且兩個都對。",
    ),
    "ch04_nakade5": dict(n=7, AB=w5, AW=o5,
        LB={r5[i]: ch for i, ch in enumerate("pqrst")},
    ...
    "ch04_ex2": dict(n=3,
        ...
        AW=["C1", "C2"],
        LB={"C3": "p"},
        comment="練習 4.2：把最小雙活改一下 —— 白少一子，多出 p 這個點。"
                "先分類 (a, b, c) 並套判定式，然後【檢查三個前提】。"
                "最後用搜尋核對。三個答案會不會一致？",
    ),
    "ch04_ex3": dict(n=7, AB=w3, AW=o3,
        LB={r3[1]: "p"},
        comment="練習 4.3：同樣的三目大眼。白第一手應該下 p（正中），還是下旁邊？"
                "兩種都算一遍，比較白總共要花幾手。",
    ),
}
